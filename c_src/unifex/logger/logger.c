...
static int (*send_log_func)(UnifexEnv *env, UnifexPid pid, int flags,
                           char const *level, char const *message, char const *time,
                           char **tags, unsigned int tags_length) = NULL;
...
void *unifex_logger_worker(void *arg) {
  ...
  while (queue.running) {
    ...
    while (queue.count > 0) {
      ...
      int get_pid_flags = msg.is_threaded ? UNIFEX_FROM_CREATED_THREAD : UNIFEX_NO_FLAGS;
      
      // Try to get the target PID
      // Note: We pass NULL as env since we're in a worker thread
      if (unifex_get_pid_by_name(NULL, target_pid_name, get_pid_flags, &target_pid)) {
        // Create an environment for this thread
        UnifexEnv *env = enif_alloc_env();
        if (env && send_log_func) {
          int send_flags = msg.is_threaded ? UNIFEX_SEND_THREADED : UNIFEX_NO_FLAGS;
          // Call the registered send function
          send_log_func(env, target_pid, send_flags, 
                       msg.level, msg.message, msg.time, 
          ...
          // Create the message tuple: {:unifex_logger, level, message, time, [tags]}
          ERL_NIF_TERM term = ({
            const ERL_NIF_TERM terms[] = {
                ...
                ({
                  ...
                  for (int i = msg.tags_length - 1; i >= 0; i--) {
                    list = enif_make_list_cell(env, enif_make_atom(env, msg.tags[i]), list);
                  }
                  list;
                })
            };
